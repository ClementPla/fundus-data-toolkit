...
def merge_existing_datamodules(datamodules: List[FundusDatamodule], separate_test_sets:bool=True) -> FundusDatamodule:
    ...
    assert all(isinstance(dm, FundusDatamodule) for dm in datamodules), 'All datamodules must be of type FundusDatamodule'
    ...
    new_datamodule = FundusDatamodule(img_size=img_size.pop(), batch_size=batch_size.pop(), num_workers=num_workers.pop())
